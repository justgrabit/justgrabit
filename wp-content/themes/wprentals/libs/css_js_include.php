...
function wpestate_admin($hook_suffix) {	
    ...
    if (is_admin() &&  ( $pagenow=='post-new.php' || $pagenow=='post.php') && $typenow=='estate_property') { 
        ...
        if($what_map==1){
            $google_maps_link_ssl   =   'https://maps-api-ssl.google.com/maps/api/js?v=3.35'.$libraries.'&key='.esc_html(wprentals_get_option('wp_estate_api_key', '') );
            $google_maps_link       =   'http://maps.googleapis.com/maps/api/js?v=3.35'.$libraries.'&key='.esc_html(wprentals_get_option('wp_estate_api_key', '') );
