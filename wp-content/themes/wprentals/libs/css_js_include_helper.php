    ...
    function wpestate_load_google_map(){
        ...
        if($what_map==1){
            if (!wp_script_is( 'wpestate_googlemap', 'enqueued' )) {
                ...
                $google_maps_link_ssl   =   'https://maps-api-ssl.google.com/maps/api/js?v=3.37'.$libraries.'&key='.esc_html(wprentals_get_option('wp_estate_api_key', '') );
                $google_maps_link       =   'http://maps.googleapis.com/maps/api/js?v=3.37'.$libraries.'&key='.esc_html(wprentals_get_option('wp_estate_api_key', '') );
