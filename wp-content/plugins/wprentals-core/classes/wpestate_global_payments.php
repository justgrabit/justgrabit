...
class Wpestate_Global_Payments {
    ...
    */
    ...
    function wpestate_fa_woo($wpestate_propid,$invoice_no,$bookid,$depozit,$is_submit,$is_featured){
        if($is_submit==1){
            if($is_featured==1){
                $title= sprintf( esc_html__('Upgrade to "Featured" for Listing "%s" with id %s', 'wprentals_core'), get_the_title($wpestate_propid),$wpestate_propid);
                $invoice_no=$wpestate_propid.'f';
            }else{
                $title= sprintf( esc_html__('Payment for Listing "%s" with id %s', 'wprentals_core'), get_the_title($wpestate_propid),$wpestate_propid);
            ...
            $post = array(
                'post_content'   => '',
                'post_status'    => "publish",
                'post_title'     => sprintf( esc_html__("Payment for Invoice %s , Booking %s", 'wprentals_core'), floatval($invoice_no) ,$bookid),
                'post_parent'    => '',
                'post_type'      => "product",
                'comment_status' => 'closed'
            );
